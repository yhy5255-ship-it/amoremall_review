...
6. 각 섹션(매출, 신규가입, 앱설치, 트래픽)마다 지정된 리드(소제목) 구조를 정확히 따르세요:
   - 매출: 리드 1개, "매출" (details 2개: 매출 성과 문장, 첫구매/회원가입 문장)
   - 신규가입: 리드 1개, "신규가입" (details 2개: 매출 성과 문장, 첫구매/회원가입 및 단가 문장)
   - 앱설치: 리드 3개, 순서대로 "앱설치", "매출", "첫구매/회원가입"
   - 트래픽: 리드 3개, 순서대로 "유입", "매출", "첫구매/회원가입"
   (매출 섹션에는 "매출_KPF 상세성과"라는 리드를 절대 만들지 마세요 - 그건 별도 규칙 기반 데이터로 이미 처리되어 클라이언트에서 추가됩니다.)
7. 각 리드 안의 "details" 배열의 원소 하나가 원래 리포트의 "· " 불릿 하나에 해당하며, 각 원소는 { text, basis } 객체입니다. "text"는 불릿 문장이고, 한 불릿 안에서 문장이 여러 개로 이어지는 경우 그 문자열 안에 줄바꿈(\\n)을 넣어 이어서 쓰세요 (접두사 없이 다음 줄에 이어지는 문장으로 렌더링됩니다).
...
- 금주 ROAS 변동에 가장 영향을 준 기획전(decliners) 또는 상위 기획전(top3, KPF 제외 - 이미 계산에서 제외되어 있음)
- 첫구매/회원가입 금주 총 발생 수

### 신규가입
- 매출 전주 대비 증감률(%p), 금주 ROAS, 주요 영향 기획전(decliners)
- 첫구매/회원가입 금주 총 발생 수
- 전주 대비 금주 첫구매 단가(cpp) 증감액 및 회원가입 단가(cpa) 증감액
...
### 트래픽
- 금주 집행 매체(mediaList), 평균 CTR/CPC
- 매출 대다수를 차지한 매체(topMediaByGmv), 전체 ROAS
- 첫구매/회원가입 금주 총 발생 수`;
