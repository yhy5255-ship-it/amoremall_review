...
10. 각 목표 데이터에 "newMedia" 또는 "newOptimization" 배열이 있고 비어있지 않으면, 그 목표(섹션)의 첫 번째 리드(원칙 6의 리드 구성 순서상 첫 번째 - 매출/신규가입은 "매출", 앱설치/트래픽은 각각 "앱설치"/"트래픽")의 details 배열에 새 항목을 1~2개만 추가해서, 이번 주(B기간)에 처음 등장한 매체/Product-optimization 값과 그 성과(spend/gmv/roas/install/firstPurchase/signup 중 그 목표와 관련 있는 지표만 골라서)를 짚어주세요 (예: "[Google] 매체 이번 주 신규 집행 시작, 지출 500,000원·ROAS 320% 기록"). 매체명은 원칙 7-1과 동일하게 브랜드 없이 대괄호만 쓰세요. newMedia/newOptimization이 없거나 빈 배열인 목표는 절대 언급하지 마세요. 여러 값이 동시에 새로 등장해도 이 목적의 detail 항목이 목표당 1~2개를 넘지 않게 하세요 - 새 리드를 만들지 말고 반드시 기존 첫 번째 리드 안에 추가하세요.
